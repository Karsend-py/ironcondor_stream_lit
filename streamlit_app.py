...
def build_blackout_mask(index: pd.DatetimeIndex, earnings: List[pd.Timestamp], pre: int, post: int) -> np.ndarray:
    """Vectorized blackout mask over index."""
...
def infer_bars_per_day(index: pd.DatetimeIndex) -> int:
    """Median count per day → bars_per_day."""
    if len(index) == 0:
        return 1
...
def compute_indicators_hourly(df: pd.DataFrame, bars_per_day: int, days_per_year: int) -> pd.DataFrame:
    """Hourly indicators: windows scaled by bars_per_day; HV annualized by sqrt(bars_per_day*days_per_year)."""
    df = df.copy()
...
def in_blackout(day, earnings: List[pd.Timestamp], pre: int, post: int):
    """Daily path per-row blackout (kept for compatibility)."""
...
def next_friday_close_within(idx: pd.DatetimeIndex, start_loc: int, max_dte=5) -> pd.Timestamp:
    """Hourly: pick last timestamp on first Friday in window (approx close)."""
...
def run_backtest_hourly(
    ...
    if not equity_df.empty:
        ...
        if max_dd_val > 0 and not running_max.empty:
            dd_idx = drawdown_mag.idxmax()
            max_dd_pct = (max_dd_val / running_max.loc[dd_idx]) * Absolutely—here’s your **full Streamlit app** with:

- **Daily/Hourly toggle** (hourly support added without touching daily logic).
- **Performance optimizations** (vectorized blackout, downsampling for charts).
- **Fixed f-string brace error** (replaced the `html(f"""...""")` preset block with Streamlit-native buttons and JSON download/upload—no raw `{}` inside f-strings).
- Same UI: uploads, settings, run button, outputs, charts, trades on one page.

Copy-paste this into your file (e.g., `streamlit_app.py`) and run it.
