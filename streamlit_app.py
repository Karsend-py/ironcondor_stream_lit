...
<span>Options analytics & strategy management</span><br/>
<small>v1.2</small>
...
# =========================================================
# >>> NEW: Minute resampler & scaled Friday-close picker
# =========================================================
def resample_to_minute_interval(df_in: pd.DataFrame, interval_min: int) -> pd.DataFrame:
    """
    Resample intraday minute data to a target minute interval.
    Expects columns: timestamp, close, high, low, vwap
    Aggregation:
      - close: last
      - high: max
      - low:  min
      - vwap: mean (approx; no volume column provided)
    """
    df = df_in.copy()
    df["timestamp"] = pd.to_datetime(df.get("timestamp"), errors="coerce")
    df = df.dropna(subset=["timestamp"]).sort_values("timestamp").set_index("timestamp")

    cols = [c for c in ["close", "high", "low", "vwap"] if c in df.columns]
    if not cols or "close" not in cols:
        # Not enough columns to resample properly; return original (sorted) frame
        return df_in.dropna(subset=["timestamp"]).sort_values("timestamp")

    agg = {"close": "last", "high": "max", "low": "min", "vwap": "mean"}
    df_res = df.resample(f"{int(interval_min)}T").agg(agg).dropna(how="any")
    df_res = df_res.reset_index()  # restore timestamp as a column
    return df_res

def next_friday_within(idx: pd.DatetimeIndex, start_loc: int, max_dte=5):
    """Daily: pick the calendar Friday within window."""
...
def next_friday_close_within(idx: pd.DatetimeIndex, start_loc: int, max_dte=5) -> pd.Timestamp:
    """Hourly (original): pick the last timestamp on the first Friday within window."""
...
def next_friday_close_within_scaled(idx: pd.DatetimeIndex, start_loc: int, bars_per_day: int, max_dte: int = 5) -> pd.Timestamp:
    """
    >>> NEW: Pick the last timestamp on the first Friday within the next max_dte days,
    using bars_per_day for the search window size (works for hour or minute bars).
    """
    bpd = max(int(bars_per_day), 1)
    end_loc = min(start_loc + max_dte * bpd, len(idx) - 1)

    fridays = [t for t in idx[start_loc:end_loc + 1] if pd.Timestamp(t).weekday() == 4]
    if not fridays:
        return idx[end_loc]
...
def compute_indicators_hourly(df: pd.DataFrame, bars_per_day: int, days_per_year: int) -> pd.DataFrame:
    """
    Hourly/minute indicator logic: scale windows by bars_per_day and annualization by bars_per_day*days_per_year.
    """
    df = df.copy()
    bpd = max(int(bars_per_day), 1)
    bb_n = 20 * bpd
...
# =========================================================
# HOURLY/MINUTE BACKTEST (optimized, same entry/exit logic)
# =========================================================
def run_backtest_hourly(
    ...
    # Indicators tuned for hourly/minute via bars_per_day
    ...
    for i in range(n):
        ...
        if eligible_arr[i]:
            ...
            # >>> NEW: use scaled Friday-close picker (works for minute/hour)
            expiry = next_friday_close_within_scaled(idx, i, bars_per_day=bars_per_day, max_dte=5)
...
with st.container():
    ...
    elif timeframe_mode == "Minute":
        st.markdown("**Minute Settings**")
        minute_interval = st.selectbox("Minute interval", options=[30, 15, 5, 1], index=3)
        # Same base settings as Hourly
        bars_per_day_override = st.number_input(
            "Bars per day (override)", value=0,
            help="If 0, the app infers from data. For equities: ~390/interval; for crypto: 1440/interval.",
    ...
    preset = {
        ...
        # >>> NEW: store minute interval if Minute mode is selected
        **({"minute_interval": minute_interval} if timeframe_mode == "Minute" else {})
    }
...
if run_clicked:
    ...
    if missing_full:
        ...
        with o1:
            st.markdown('<b>Trades</b><br/>—', unsafe_allow_html=True)
        with o2:
            st.markdown('<b>Win Rate</b><br/>—', unsafe_allow_html=True)
        with o3:
            st.markdown('<b>Total P&L</b><br/>$—', unsafe_allow_html=True)
        with o4:
            st.markdown('<b>Max Drawdown</b><br/>$—<br/>—%', unsafe_allow_html=True)
    ...
    with st.spinner("Running backtest…"):
        ...
        else:  # >>> NEW: Minute branch
            # 1) Resample to selected minute interval (safe if CSV is already aggregated)
            df_min = resample_to_minute_interval(df_sorted, int(minute_interval))

            # 2) Infer or use override bars_per_day (number of bars per calendar day)
            idx_sorted_min = pd.DatetimeIndex(df_min["timestamp"].values)
            bpd_inferred = infer_bars_per_day(idx_sorted_min)
            bars_per_day = bars_per_day_override if int(bars_per_day_override) > 0 else bpd_inferred

            # 3) Reuse the hourly backtest with proper scaling for minute bars
            df_out, trades_df, equity_df, summary = run_backtest_hourly(
                df_raw=df_min,
                blackout_dates=blackout_dates,
                hv_min=hv_min, hv_max=hv_max,
    ...
    r1c1.markdown(f"<b>Trades</b><br/>{summary['trades']}", unsafe_allow_html=True)
    r1c2.markdown(f"<b>Win Rate</b><br/>{summary['win_rate']:.2f}%", unsafe_allow_html=True)
    r1c3.markdown(f"<b>Total P&L</b><br/>${summary['total_pnl']:.2f}", unsafe_allow_html=True)
    r1c4.markdown(f"<b>Max Drawdown</b><br/>${summary['max_drawdown']:.2f}<br/>{summary['max_drawdown_pct']:.2f}%", unsafe_allow_html=True)
    r1c5.markdown(
        f"<b>Exits (Bre/ADX/VWAP/Broke)</b><br/>{summary['breaches']}/{summary['adx_exits']}/{summary['vwap_exits']}/{summary['brokes']}",
        unsafe_allow_html=True
    )
    ...
    r2c1.markdown(f"<b>Current Drawdown</b><br/>${abs(dd_current_val):,.2f}<br/>{abs(dd_current_pct):.2f}%", unsafe_allow_html=True)
    r2c2.markdown(f"<b>Max Drawdown</b><br/>${summary['max_drawdown']:,.2f}<br/>{summary['max_drawdown_pct']:.2f}%", unsafe_allow_html=True)
    r2c3.markdown(f"<b>Peak Equity</b><br/>${peak_equity:,.2f}", unsafe_allow_html=True)
    ...
    r3c1.markdown(f"<b>Total Loss</b><br/>${total_loss:,.2f}", unsafe_allow_html=True)
    r3c2.markdown(f"<b># Losing Trades</b><br/>{num_losses}", unsafe_allow_html=True)
    r3c3.markdown(f"<b>Avg Loss</b><br/>${avg_loss:,.2f}", unsafe_allow_html=True)
    r3c4.markdown(f"<b>Worst Loss</b><br/>${worst_loss:,.2f}", unsafe_allow_html=True)
