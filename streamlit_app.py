...
# === NEW: Caching for file reading and blackout parsing ===
@st.cache_data(show_spinner=False)
def load_csv(file) -> pd.DataFrame:
    return pd.read_csv(file, dtype={"close": "float64", "high": "float64", "low": "float64", "vwap": "float64"}, parse_dates=["timestamp"])

@st.cache_data(show_spinner=False)
def load_blackout(file) -> list[pd.Timestamp]:
    return parse_blackout_txt(file)
...
if run_clicked:
    ...
    blackout_dates = load_blackout(txt_file)
